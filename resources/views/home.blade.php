...
@section('content')
    <div>
        ...
        <table>
            @foreach([
                'trader' => 'Most Prolific Trader', 
                'longestWin' => 'Longest Winning Streak', 
                'longestLoss' => 'Longest Loosing Streak' 
                ] as $key => $value)
                <tr>
                    <td><strong>{{ $value }}</strong></td>
                    <td><strong>{{ $$key['name'] }}</strong> ({!! $$key['value'] !!})</td>
                </tr>
            @endforeach
        </table>
        <h2>Match Stats</h2>
        <table>
            @foreach([
                'biggest' => 'Biggest Winning Margin', 
                'highest' => 'Highest Scoring Match', 
                'lowest' => 'Lowest Scoring Match'
                ] as $key => $value)
                <tr>
        ...
        @foreach([
            ...
            'undervalued' => 'Most Undervalued Player', 
            'worst' => 'Worst Player', 
            'keeper' => 'Best Keeper', 
            'defender' => 'Best Defender', 
            'midfielder' => 'Best Midfielder', 
